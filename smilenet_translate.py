...
class fixUp(torch.autograd.Function):
      
    @staticmethod
    def forward(ctx, coeffs, bounds):
        tol = 1.0e-5 # Tolerance for discarding imaginary roots

        # 1. Convert into polynomials from output:
        cond_coeffs = transform(coeffs)
        polys = [np.polynomial.polynomial.Polynomial(cond_coeffs[i,:]) for i in range(cond_coeffs.shape[0])]

        # 2. Find minima of function
        lowest = 0
        for poly, (x0, x1) in zip(polys, zip(bounds[:-1],bounds[1:])):
            # print("------------")
            # print(poly.coef)
            polyder = poly.deriv()
            roots = polyder.roots() # For each root of derivative, we have to evaluate a minima
            roots = roots.real[abs(roots.imag) < tol]
            roots = roots[(roots >= x0) & (roots <= x1)]
            lowest = min(lowest, min(map(lambda x: poly(x), [x0] + list(roots) + [x1])))
        
        translate_coeff = _ # figure out in a sec
        addition_vec = torch.zeros((coeffs.shape[0] // 4, 4))
        addition_vec[:,3] = lowest / translate_coeff
        addition_vec = addition_vec.flatten()
        coeffs = coeffs + addition_vec
        return coeffs
