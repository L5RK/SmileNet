...
def get_xstar(basis, knots, strikes, mids, ini_sys, spreads=None):
    ...
    X = spreads @ (eqs @ basis)
    lamb = 1.0e-3
    bsol  = torch.linalg.lstsq( X.T @ X + torch.eye(X.shape[1]).double() * lamb, X.T @ spreads @ mids.reshape(-1,1))[0]
    sol = basis @ bsol
    polys = [np.polynomial.polynomial.Polynomial(list(sol[:,0].detach().numpy()[4*i:4*i+4])) for i in range((sol.shape[0])//4)]
    plot_polys([(lambda x, p=poly: p(x) ** 2) for poly in polys], torch.hstack([torch.tensor(strikes[0]), knots, torch.tensor(strikes[-1])]), 0, 'before', extra_points=zip(strikes, mids ** 2))
    return bsol
...
class SmileNet(torch.nn.Module):
    ...
    def __init__(self, num_knots, strikes, mids):
        ...
        strikes = torch.sqrt(self.s * strikes + self.t)
        initial_knots = torch.sqrt(self.s * initial_knots + self.t)
    ...
    def forward(self, x, scale=None):
        ...
        if self.memo is None:
            ...
            mids = (his + lows) / 2
            spreads = his-lows
            strikes = torch.sqrt(self.s * flatx[2*(flatx.shape[0] - 3)//3:-3] + self.t)
            init_basis, ini_sys = get_basis(self.base_system, self.knot_skeleton.reshape(-1), self.seeded_vecs)

            xstar = get_xstar(init_basis, self.knot_skeleton.reshape(-1), strikes,mids , ini_sys, spreads=spreads)
            self.memo=xstar
        else:
            xstar = self.memo
        ...
        if scale is None:
            scale = self.scale
        translate, sys = get_basis(self.base_system, knots[0,:], self.seeded_vecs)
        ...
        control1, l = removeArb(control.T, translate, knots, self.strike_low, self.strike_high, self.s, self.t, sys=sys)

        sol = translate @ control1
        sol = getBlockForm(sol)
        # print(sol)
        sol = torch.hstack([differentiablePolyMul.apply(sol[i,:], sol[i,:], 6) for i in range(sol.shape[0])]).reshape(-1,1)
    ...
    def train(self, data, strikes, epochs=1000, optimizer=None):
        ...
        if optimizer is None:
            optimizer = torch.optim.Adamax(self.parameters(), lr=1.0e-2)
        ...
        for e in range(epochs):
            ...
            transformed = transform(pred, self.s, self.t)
            ...
            if True: #e%10==0:
                print('it',e,'plotted')
                plot_polys([poly for poly in polys], torch.hstack([torch.tensor(self.strike_low) , boundaries, torch.tensor(self.strike_high)]), e, 'it', extra_points=[(strikes, lows), (strikes, highs)], title=str(loss.item()))
...
def transform(sol, s=0.1, t=1, scale=1):
    # Assume sol is of form a0, b0, c0, d0, a1, b1, ...
    transforms = []
    sol = sol
    sol_degree = sol.shape[1] - 1
    max_degree = sol_degree *2 + (sol_degree - 1) * 2 + 1 + 1 # Extra for u 
    print('it')
    for i in range(0, sol.shape[0]):
        # print(i)
        # print(np.polynomial.polynomial.Polynomial(sol[i,:].detach()))
        ...
        a = sol[i,:]
        b = differentiablePolyDeriv.apply(a) * s
        c = differentiablePolyDeriv.apply(b) * s ** 2

        a2 = preservingPolyMul(a, a)
        b2 = preservingPolyMul(b,b)
        u = torch.tensor([0,1])
        u2 = torch.tensor([0,0,1])
        u3 = torch.tensor([0,0,0,1])
        k = torch.tensor([-t/s,0,1.0/s]) # Account for translation
        k2 = preservingPolyMul(k,k)

        # Corresponding to u **3 w(u) ** 2 (1 - w'(u) k / (4 * u * w(u))) ** 2
        cond = preservingPolyMul(u2, a2)
        cond = preservingPolySub(cond, 0.5 * preservingPolyMul(preservingPolyMul(a,b),k))
        cond = preservingPolyAdd(cond, preservingPolyMul(k2, b2) / 16.0)
        cond = preservingPolyMul(cond, u)
        # print('1')
        # print(cond)        
        # Corresponding to w(u) ** 2 u ** 3 ( w'(u)** 2 / 16u^2 (1/w(u) + 1/4))
        
        term = preservingPolyMul(preservingPolyMul(a2, b), u) / 16.0
        term = preservingPolyAdd(preservingPolyMul(preservingPolyMul(a2, b2), u) / 32.0, term)
        cond = preservingPolySub(cond, term)
        # print('2')
        # print(cond)
        # corresponding to w(k)**2 * u** 3 * -w''(k)/ 8u**3
        cond = preservingPolySub(cond, preservingPolyMul(c, a2) / 8.0)
...
class differentiablePolyRoots(torch.autograd.Function):
    ...
    def forward(ctx, coeffs, x0, x1):
        ...
        roots = torch.hstack([torch.tensor(x0), roots, torch.tensor(x1)])
    ...
    def backward(ctx, grad_out):
        ...
        ders = torch.zeros((roots.shape[0], coeffs.shape[0])).double()
        der = np.polynomial.polynomial.Polynomial(coeffs.detach()).deriv()
        # Jacobian (dy/dxi ...)
        # droots / dcoeffi
        for i in range(1, roots.shape[0] - 1):
...
def polyLossT(cond_coeffs, bounds, sl, sh):
    ...
    for poly, (x0, x1) in zip(polys, zip(bounds[0,:-1],bounds[0,1:])):
        ...
        for point in roots:
            loss = loss + torch.min(torch.tensor(0.0), differentiablePolyEval.apply(poly, point))
    return -1 * loss
...
def get_observe_losses(coeffs, boundaries, strikes, lows, highs):
    ...
    polys = [lambda x, p=coeffs[i,:]: (differentiablePolyEval.apply(p, x)) for i in range((coeffs.shape[0]))]
    last_bound = 0
...
def removeArb(control, translate, boundaries, sl, sh, s, t, sys=None):
    lr = 0.1
    gamma = 0.9
    tol=1.0e-5
    max_epochs = 100
    ...
    for e in range(max_epochs):
        ...
        for p in polys:
            print(p)
        plot_polys(polys, torch.hstack([torch.tensor(sl) , boundaries.reshape(-1), torch.tensor(sh)]), e, 'pint','p')        
        plot_polys([poly.deriv(1) for poly in polys], torch.hstack([torch.tensor(sl) , boundaries.reshape(-1), torch.tensor(sh)]), e, 'pint1_','p')        
        plot_polys([poly.deriv(2) for poly in polys], torch.hstack([torch.tensor(sl) , boundaries.reshape(-1), torch.tensor(sh)]), e, 'pint2_','p')        
        # sol = sol0
        # print(sol0)
        ...
        # raise Exception("D")
        transformed = transform(sol, s, t, scale=1.0)
        # print("-------")
        ...
        # for i, j , k in zip(tpolys, polys,polys2) :
        plot_polys(tpolys, torch.hstack([torch.tensor(sl) , boundaries.reshape(-1), torch.tensor(sh)]), e, 'tint','t')        
        polys = [np.polynomial.polynomial.Polynomial(sol0.detach().numpy()[i,:])  for i in range(sol0.shape[0])]
        ...
        loss = polyLossT(transformed, boundaries, sl, sh) # + scale * polyLoss.apply(sol, boundaries, sl, sh)
        ##########################
        # print(torch.autograd.grad(loss, sol, retain_graph=True))
        ...
        print(max(grad))
        control = control - lr * (1+grad)   # TODO differentiable_update
        lr = lr * gamma
