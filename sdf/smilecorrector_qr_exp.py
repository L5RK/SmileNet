...
class SmileNet(torch.nn.Module):
    ...
    def forward(self, x, scale=None):
        ...
        # control1, l = removeArb(control.T, translate, knots, self.strike_low, self.strike_high)
...
def crudePolyCorrect(ocoeffs, boundaries, sl, sh):
    ...
    for e in range(max_epochs):
        ...
        # optimizer.step()
        # optimizer.zero_grad()
        grad = torch.autograd.grad(loss, ocoeffs, create_graph=True)[0]
        ocoeffs = ocoeffs - lr * grad # TODO differentiable_update
    print(type(ocoeffs))
    return ocoeffs
...
def removeArb(control, translate, boundaries, sl, sh):
    ...
    for e in range(max_epochs):
        ...
        plot_polys(ppolys, torch.hstack([torch.tensor(sl) , boundaries.reshape(-1), torch.tensor(sh)]), e, 'pitn','v')        
