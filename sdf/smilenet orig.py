...
def ind(a,b,c,d):
    return a + 4 * b + 4 * 4 * c + 4 * 4 * 4 * 3 * d

def get_dummy():

    dummy = torch.zeros((4*4*4*4), 11).float() 
...
def loss_on_spline(coeffs, boundaries):
    ...
    coeff_pows = torch.tensor([coeffs]).T ** torch.tensor([0,1,2,3,4])

    depth, size = coeff_pows.shape[0], coeff_pows.shape[1]
    combs = coeff_pows[0]
    for i in range(1, depth):
        combs = coeff_pows[i] * combs

    combs = combs.flatten()

    dummy = get_dummy()
    
    poly = (combs @ dummy()).flip((0)) # Need highest degree first
    roots = torch.tensor(np.roots(poly))
    new_powers = torch.tensor([i for i in range(poly.shape[0],0,-1)])
    integral = poly / new_powers
    roots, _ = torch.sort(roots.real[abs(roots.imag) < threshold])
...
def loss_on_splines(coeffs, boundaries):
    loss = torch.tensor(0)
    for i in range(coeffs.shape[0] // 4 + 1):
        loss += loss_on_spline(coeffs[i:i+4], (boundaries[i], boundaries[i+1]))
    return loss
...
class SmileNet(torch.nn.Module):
    ...
    def __init__(self, boundaries, strikes, mids):
        ...
        for i in range(len(boundaries)):
            if boundaries[i] == strikes[count]:
                strike_knots += [(i,mids[count])]
                count += 1

        basis, X = get_translation_matrix(boundaries, dof)
        for i in range(X.shape[0]):
            print(X[i,:])
        xstar = torch.tensor(simple_initialization(mids, strike_knots, dof, basis, X))
        ...
        self.blackbox = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(),
            torch.nn.Linear(strike_num * 4, dof),
        )
    ...
    def forward(self, x):
        ...
        print('Tranlsation', self.translate)
        return control @ self.translate # Outputs c oefficients
