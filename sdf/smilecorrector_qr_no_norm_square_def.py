...
def get_system_matrix(num_knots):
    ...
    system = torch.zeros(((num_knots * 3 + 2), (4 * (num_knots + 1)))).double()
...
def get_basis(system, knots, seeded_vecs, k_start, k_end):
    ...
    kp4 = (k_start ** p3).repeat(1, knots.shape[0] + 1)
    kp5 = (k_end ** p3).repeat(1, knots.shape[0] + 1)
    knot_powers = torch.hstack([kp1,kp2,kp3]).reshape(-1, (knots.shape[0] + 1) * 4)
    new_sys = knot_powers * system[:-2,:]
    print(system[-2:,:].shape, kp4.shape, kp5.shape, torch.vstack([kp4, kp5]).shape)
    new_sys = torch.vstack([new_sys, torch.vstack([kp4, kp5]) * system[-2:,:]])
...
def get_vector_seed(num_knots):
    dof = num_knots  + 4 - 2# Subject to change -> Linear wings -4 for linear wings
...
class SmileNet(torch.nn.Module):
    ...
    def __init__(self, num_knots, strikes, mids):
        ...
        dof = num_knots + 4  - 2# Linear wings -> -4 degrees of freedom
        strike_knots = []
        count = 0
