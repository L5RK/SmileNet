...
def get_xstar(basis, knots, strikes, mids, ini_sys, spreads=None):
    ...
    plot_polys([(lambda x, p=poly: p(x)) for poly in polys], torch.hstack([torch.tensor(-0.5), knots, torch.tensor(0.25)]), 0, 'before', extra_points=zip(strikes, mids))
    return bsol
...
def transform(sol, scale=1):
    ...
    max_degree = sol_degree *2 + (sol_degree - 1) * 2 + 1
    for i in range(0, sol.shape[0]):
        ...
        # Corresponding to 4 * w(k)**2 * (1 - w'(k)k/(2w(k))) ** 2
        cond = preservingPolyMul(preservingPolyMul(b, b), k2)
        cond = preservingPolyAdd(cond, 4.0 *a2)
        cond = preservingPolySub(cond, 4 * preservingPolyMul(preservingPolyMul(a, b), k))

        # Corresponding to 4 * w(k) ** 2 * (w'(k)**2/4 (1/w(k) + 1/4))
        cond = preservingPolySub(cond, preservingPolyMul(a, b2))
        cond = preservingPolySub(cond, 0.25 * preservingPolyMul(a2, b2))

        # corresponding to 4 * w(k) * w''(k)/ 2
        cond = preservingPolyAdd(cond, preservingPolyMul(a2, c) * 2.0)
...
def removeArb(control, translate, boundaries, sl, sh, sys=None):
    ...
    for e in range(max_epochs):
        ...
        # sol = sol0
        sol = [sol0[i,:].reshape(1,-1) for i in range(sol0.shape[0])]
