...
def get_xstar(basis, knots, strikes, mids, ini_sys, low, spreads=None):
    ...
    mids = torch.sqrt(torch.tensor(mids**2 - low))
...
class SmileNet(torch.nn.Module):
    ...
    def train(self, data, strikes, epochs=1000, optimizer=None):
        ...
        if optimizer is None:
            optimizer = torch.optim.Adam(self.parameters(), lr=5.0e-5)
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=(lambda x: 1.005**(min(x, 100))))
        S = data[0,-3]
        T = data[0,-2]
        R = data[0,-1]
        losses = []
        for e in range(epochs):
            ...
            sol, bounds, low, knots = self.forward(data, scale)
            # print('bounds vs ks')
            # print(bounds)
            ...
            if e%200 == 0:
                ...
                plot_polys(plottable_fs, ba.detach(), e, 'func','p',extra_points=[(strikes, lows), (strikes, highs)])        
            ...
            # print('Area',area)
            arb_loss = 1_000 * arb_loss 
            bounds_loss, d, mape = get_observe_losses(fs, bounds[1:-1], strikes, lows, highs)
            bounds_loss = 1.0e-4 * bounds_loss
            # print('bounds')
            # print(bounds)
            # print(torch.autograd.grad(bounds_loss, knots, retain_graph=True))
...
def get_observe_losses(fs, boundaries, strikes, lows, highs):
    ...
    for strike, low, high in zip(strikes, lows, highs):
        ...
        if last_mid is not None:
            ...
            delta_loss = delta_loss + ((mid_delta - func_delta) / mid_delta)**2 * torch.exp(torch.abs(strike - last_strike))
        last_mid = mid
        last_pred = predicted
        last_strike = strike
        out = (high - low) / 2.0 # how far is bid/ask
        err = (predicted - mid).abs() 
        mape += (torch.max(err-out, zero) / out)
        loss = loss + ((torch.max(err - out, zero) / out))** 2 / (out ** 0.5)
    delta_imp = 5
    return (delta_loss*delta_imp + loss) / len(strikes), delta_loss.item() / len(strikes), mape / len(strikes)
