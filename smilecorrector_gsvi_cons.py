...
class SmileNet(torch.nn.Module):
    ...
    def train(self, data, strikes, epochs=1000, optimizer=None):
        ...
        stat = [1]
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=(lambda x, stateful=stat: stateful[0]*1.00025**(min(x, 100))))
        S = data[0,-3]
        T = data[0,-2]
        R = data[0,-1]
        losses = []
        ...
        for e in range(epochs):
            ...
            bounds_loss = (raw_loss + delta_loss ) / len(strikes)
            
            print(delta_chng, raw_chng)
            print('Raw,delta', raw_loss, delta_loss)
            if sum(raw_window) < sum(delta_window) and raw_loss != 0:
                delta_imp = delta_imp / 1.1
    ...
    def pred_as_function(self, x):
        pred, boundaries, low, knots = self.forward(x)
        ...
        w = lambda k, buckets=buckets, fs=fs: fs[np.where((buckets[:-1] < k) & (buckets[1:] > k))](k) 
        return w
...
def get_observe_losses(fs, boundaries, strikes, lows, highs, delta_imp):
    ...
    return loss, delta_loss, delta_first, delta_last, mape / len(strikes)
