...
def get_linear_estimate(strikes, mids):
    # Just a least squares wrapper:
    # Model: a0 + a1x 
    A = torch.vstack([torch.ones(strikes.shape), strikes]).T
    y = mids.reshape(-1,1)
    return torch.linalg.lstsq(A,y) # Shape (2,1)


def get_best_estimates(basis, knots, strikes, mids, ini_sys, spreads=None):
    # Returns:
    #   1. A linear model for the left wing of the _variance_ curve
    #   2. A polynomial model for the _volatility_ curve towards the center
    #   3. A linear model for the right wing of the _variance_curve
...
def stitch_estimate(linear, polynomial, knots, l=False):
    # Takes:
    #   1. Linear model of the wing of the variance curve
    #   2. [Polynomial] representing the main curve
    #   3. List of all knots
    #   4. l - Left wing or right wing

    # For a left stitch, we stitch L to S at knot 0, S to P at knot 1
    
    # Setup:
    p_degree = 6
    powers = torch.arange(p_degree + 1)
    
    last = 0 if l else -1
    penul = 1 if l else -2

    kp1 = (knots[penul] * torch.ones(p_degree + 1)) ** powers
    kp2 = (knots[last] * torch.ones(p_degree + 1)) ** powers
    p_tensor = torch.tensor([1, 1, 1, 1, 1, 1, 1])
    p1_tensor = torch.tensor([0, 1, 2, 3, 4, 5, 6])
    p2_tensor = torch.tensor([0, 0, 2, 6, 12, 20, 30])
    l_tensor = torch.tensor([1,1])
    ld_tensor = torch.tensor([0,1])

    equalities = torch.zeros(6 ,2 * (p_degree + 1) + 2) # Spaces for: Polynomial[-1], Stitching Polynomial, Linear model
                                                        # Denote as P, S, L
    equalities[0,:(p_degree + 1)] = p_tensor * kp1
    equalities[0, (p_degree + 1):2*(p_degree+1)] = -1 * p_tensor * kp1 # Above two lines enforce P(x1) = S(x1)

    equalities[1,:(p_degree + 1)] = p1_tensor * torch.roll(kp1, shift=1)
    equalities[1, (p_degree + 1):2*(p_degree+1)] = -1 * p1_tensor * torch.roll(kp1, shift=1) # Above two lines enforce P'(x1) = S'(x1)

    equalities[2,:(p_degree + 1)] = p2_tensor * torch.roll(kp1, shift=2)
    equalities[2, (p_degree + 1):2*(p_degree+1)] = -1 * p2_tensor * torch.roll(kp1, shift=2) # Above two lines enforce P''(x1) = S''(x1)

    equalities[3, (p_degree + 1):2*(p_degree+1)] = p_tensor * kp2 
    equalities[3, 2*(p_degree+1):] = -1 * l_tensor * kp2[:2] # Above line enforces S(x0) = L(x0)

    equalities[4, (p_degree + 1):2*(p_degree+1)] = p1_tensor * torch.roll(kp2 , shift=1)
    equalities[4, 2*(p_degree+1):] = -1 * l_tensor * torch.roll(kp2, shift=1)[:2] # Above line enforces S'(x0) = L'(x0)

    equalities[5, (p_degree + 1):2*(p_degree+1)] = p2_tensor * torch.roll(kp2 , shift=2) # Above line enforces S''(x0) = 0 = L(x0)

    # Now we have our equalities matrix, we are in a position to find a solution for S(x), which is a polynomial of degree 6 subject to 5 constraints,
    # defined directly (as opposed to our input polynomials, which are defined as (p(x))^2, where p(x) is an degree 3 polynomial)
...
class SmileNet(torch.nn.Module):
    ...
    def __init__(self, num_knots, strikes, mids):
        ...
        init_basis, ini_sys = get_basis(self.base_system, initial_knots[2:-2], self.seeded_vecs, initial_knots[[0]], initial_knots[[1]])
        print(ini_sys)
    ...
    def forward(self, x, scale=None):
        ...
        # want to get left linear, right linear, based on control
        temp = getBlockForm(translate @ control.T)
        lg = differentiablePolyDeriv.apply(tempt[0],knots[1]) 
