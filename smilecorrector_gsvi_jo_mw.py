...
class SmileNet(torch.nn.Module):
    ...
    def train(self, data, strikes, epochs=1000, optimizer=None):
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=(lambda x: 1.0025**(min(x, 100))))
        S = data[0,-3]
        T = data[0,-2]
        R = data[0,-1]
        losses = []
...
def get_observe_losses(fs, boundaries, strikes, lows, highs):
    ...
    for strike, low, high in zip(strikes, lows, highs):
        ...
        loss = loss + ((torch.max(err - out, zero) / mid))** 2 / (out ** 0.5)
    return delta_loss, loss, mape / len(strikes)
