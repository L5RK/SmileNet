...
def get_system_matrix(num_knots):
    ...
    system = torch.zeros(((num_knots * 3), (4 * (num_knots + 1)))).double()
...
def get_vector_seed(num_knots):
    dof = num_knots + 4 # Subject to change -> Linear wings -4 for linear wings
...
class SmileNet(torch.nn.Module):
    ...
    def __init__(self, num_knots, strikes, mids, low):
        ...
        dof = num_knots + 4 # Linear wings -> -4 degrees of freedom
        strike_knots = []
        count = 0
    ...
    def train(self, data, strikes, epochs=1000, optimizer=None):
        ...
        for e in range(epochs):
            ...
            print(torch.autograd.grad(bounds_loss, sol, retain_graph=True)[0][2,:])
            loss = arb_loss + bounds_loss  + torch.min(bounds_loss, torch.tensor(1.0)) ** arb_loss
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            scheduler.step()
...
def get_observe_losses(fs, boundaries, strikes, lows, highs):
    ...
    for strike, low, high in zip(strikes, lows, highs):
        ...
        if last_mid is not None:
            ...
            delta_loss = delta_loss + ((mid_delta - func_delta) / mid_delta)**2
        last_mid = mid
        last_pred = predicted
    ...
    delta_imp = 1.0
    return (delta_loss*delta_imp + loss) / len(strikes)
