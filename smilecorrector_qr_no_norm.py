...
def get_xstar(basis, knots, strikes, mids, ini_sys, spreads=None):
    ...
    # print(mids)
    # print(knots)
    # print(eqs.shape, basis.shape, mids.shape)
...
class SmileNet(torch.nn.Module):
    ...
    def __init__(self, num_knots, strikes, mids):
        ...
        # print(init_basis)
        # print(init_basis.shape, ini_sys.shape, 'test')
        ...
        # i_bias[0] += (strikes[0] - strikes[1]) / 2 - strikes[0ge]
    ...
    def forward(self, x, scale=None):
        ...
        control1, l = removeArb(correction, translate, knots, self.strike_low, self.strike_high)
        print('arb grad')
        print((torch.autograd.grad(control1[0], control, retain_graph=True)[0]))
        sol = translate @ control1
        sol = getBlockForm(sol)
        sol = [torch.flip(differentiablePolyMul.apply(sol[i,:], sol[i,:], 23), dims=(0,)) for i in range(sol.shape[0])]
        sol = torch.hstack(sol).reshape(-1,1)


        # print(torch.autograd.grad(sol[16], correction, retain_graph=True) )
        ...
        # print(sol)
        # raise Exception("Deliberate")
        # sol = torch.hstack([torch.tensor([0,0,0,0.005]) for i in range(sol.shape[0]//2) ]).reshape(-1,1)
        return sol, knots 
    ...
    def train(self, data, strikes, epochs=1000, optimizer=None):
        ...
        if optimizer is None:
            optimizer = torch.optim.Adamax(self.parameters(), lr=1.0e-1)
...
def getBlockForm(poly):
    newPolys = torch.flip(poly.reshape((-1,4)),dims=(1,))
    return newPolys


def flipOrient(poly):
    newPolys = torch.flip(poly.reshape((-1, 4)), dims=(1,))
    return newPolys.reshape(poly.shape)
...
def transform(sol, scale=1):
    ...
    for i in range(0, sol.shape[0], 4):
        ...
        # max_degree = w.degree() * 2 + (w.deriv()).degree() * 2 # Without squaring - degree 3 * 2 + degree 3 * 2 - 1 = degree 23
        max_degree = 10
        ...
        k = torch.tensor([-1,1]) # Account for translation
...
class differentiablePolyRoots(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        print(grad_out.shape, ders.shape)
...
def crudePolyCorrect(ocoeffs, boundaries, sl, sh):
    ...
    polys = torch.flip(ocoeffs.reshape((-1, 4)), dims=(1,))
    ...
    def forward(ctx, cond_coeffs, bounds, sl, sh):
        ...
        for poly, (x0, x1) in zip(polys, zip(bounds[0,:-1],bounds[0,1:])):
            ...
            for lo, hi in zip([x0] + list(roots), list(roots) + [x1]):
                ...
                integral = poly.integ()
                if min(0, integral(hi) - integral(lo)) < 0:
                    print(lo, hi, min(0, integral(hi) - integral(lo) - 1.0e-6) ,poly(hi), poly(lo))
...
def removeArb(control, translate, boundaries, sl, sh):
    lr = 5.0e-1
    tol=1.0e-6
    max_epochs = 100
    scale=1
    ...
    for e in range(max_epochs):
        ...
        polys = torch.flip(sol.reshape((-1, 4)), dims=(1,))

    
        loss = scale * polyLossT(transformed, boundaries, sl, sh) # + scale * polyLoss.apply(sol, boundaries, sl, sh)
        ##########################
        print('Arb loss', loss)
        ...
        # optimizer.step()
        # optimizer.zero_grad()
        grad = torch.autograd.grad(loss, control, create_graph=True)[0]
        # print("@@@@@@@@@@@@@@@@@@")
        # print(torch.autograd.grad(loss, sol,retain_graph=True))
        # print(transformed[0,:])
        # print(torch.autograd.grad(transformed[0,1], sol, retain_graph=True))
        # print("end @")
        control = translate.T @ crudePolyCorrect(translate @ (control - lr * grad), boundaries, sl, sh)  # TODO differentiable_update
